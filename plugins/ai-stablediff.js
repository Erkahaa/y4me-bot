...
const handler = async (m, { conn, args, text, usedPrefix, command, quoted }) => {
  if (!text) throw `
...
`
  ...
  const res = await (await fetch(`https://api.itsrose.life/image/stable/diffusion?prompt=${prompt}&negative_prompt=bad%20anatomy&ratio=9%3A16&cfg=7.5&model_id=midjourney&seed=123456&apikey=${global.rose}`)).buffer();
