...
const handler = async (m, { conn, args, usedPrefix, command }) => {
  try {
    ...
    if (!/audio/.test(mime)) throw `Reply to an audio message with the *${usedPrefix + command}* command`;
    ...
    let media = path.join(String(__dirname), '../tmp/' + ran);
let filename = media + '.mp3';
    ...
    exec(`ffmpeg -i ${media} ${set} ${filename}`, async (err) => {
      ...
      conn.sendAudio(m.chat, buff, null, m, { mimetype: 'audio/mp4', ptt: true, quoted: m });
...
handler.tags = ['audio'];
handler.help = ['bass', 'blown', 'deep', 'earrape', 'fast', 'nightcore', 'reverse', 'robot', 'slow', 'smooth', 'tupai', 'squirrel', 'chipmunk', 'vibra'];
