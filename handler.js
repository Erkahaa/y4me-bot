        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                    let chat = global.db.data.chats[m.chat];
let user = global.db.data.users[m.sender];
if (name !== 'owner-unbanchat.js' && chat?.isBanned && !isOwner) 
    return// Except this
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `✳️ Berlian Anda terjual habis\ngunakan perintah berikut untuk membeli lebih banyak berlian \n*${usedPrefix}buy* <jumlah> \n*${usedPrefix}buyall*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `✳️ tingkat yang diperlukan ${plugin.level} untuk menggunakan perintah ini. \nLevel mu ${_user.level}`, m)
                ...
                try {
                    ...
                    if (m.diamond)
                        m.reply(`kamu gunakan *${+m.diamond}* 💎`)
                }
                break
            }
        }
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Selamat datang, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Annonymous') :
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user telah diangkat menjadi admin')
        case 'demote':
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        mods: '🔰 Perintah ini khusus untuk moderator',
        ...
        private: '📮 Perintah ini hanya dapat digunakan di *obrolan pribadi Bot*',
        admin: '🛡️ Perintah ini hanya untuk *Admin* grup',
        botAdmin: '💥 Untuk menggunakan perintah ini saya harus menjadi *Administrator!*',
        unreg: '📇 Daftar untuk menggunakan fitur ini Dengan menulis:\n\n*/reg name.age*\n\n📌Contoh: */reg erka.16*',
        restrict: '🔐 Perintah ini Di disable oleh owner*'
    }[type]
...
watchFile(file, async () => {
    unwatchFile(file)
    console.log(chalk.magenta("✅  Handler telah diperbarui'handler.js'"))
