...
import { ffmpeg } from './converter.js'; // Adjust your import path
import { spawn } from 'child_process';
import uploadFile from './uploadFile.js'; // Adjust your import path
import { fromBuffer } from 'file-type';
import uploadImage from './uploadImage.js'; // Adjust your import path
...
async function sticker(img, url, packname, author) {
  if (url) img = (await (await fetch(url)).buffer()).toString('base64');
  let buffer = await conn.customSticker(packname, author, 'sticker', img);
  return buffer;
}
